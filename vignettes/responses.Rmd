...
installed qsurvey; see the [insallation
instructions](http://jdunham.io/qsurvey/).
...
Let's take a look at the columns in the response data. The first eleven are
survey and respondent metadata. If they aren't useful to us, we can drop them
with the `drop_meta()` function. We're left with actual survey responses and the
eight columns named `DO-*` that give question display orders. 
